-- =====================================================================
-- Pinecone 数据库完整初始化脚本（合并版）
-- 功能：建库 → 建用户 → 完整表结构（23 表 + 4 枚举，含历史直接 DDL 漂移列）
-- 来源：pg_dump --schema-only 从当前开发库导出（2026-08-06）
-- 用法：
--   ① 以超级用户连接（如 postgres）执行本脚本
--   ② 执行前将下方 <APP_PASSWORD> / <ADMIN_PASSWORD> 替换为你的强密码
--      （或直接沿用 .env 里的值）
-- 注意：本脚本会 CREATE DATABASE，必须用 postgres 超级用户执行，
--       不能用 pinecone_admin（它无权创建数据库）
-- =====================================================================
...
-- ── 3. 建用户 ──
-- pinecone_admin：DDL 权限（建表/迁移用），生产建议设为 SUPERUSER 或给 CREATEDB
CREATE USER pinecone_admin WITH PASSWORD '<ADMIN_PASSWORD>' CREATEDB;
-- pinecone_app：DML 最小权限（应用运行时用）
CREATE USER pinecone_app WITH PASSWORD '<APP_PASSWORD>';

-- ── 4. schema 所有权 ──
ALTER SCHEMA public OWNER TO pinecone_admin;

-- ── 5. 完整表结构（真实 schema，23 表 + 4 枚举）──
